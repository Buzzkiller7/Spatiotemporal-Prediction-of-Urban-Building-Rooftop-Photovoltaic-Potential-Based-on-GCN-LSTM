...
1. 读取子图数据汇总的数据表，只保留有聚类标签的子图，然后按照聚类标签进行分组。
2. 对于每个季节 (spring, summer, autumn, winter):
    a. 循环处理每个聚类标签的子图组：
        - 读取子图的邻接列表和太阳能数据。
        - 进行数据预处理, 包括计算节点数、边数等, 创建GraphInfo实例,并将数据集划分为训练集和测试集。
        - 运行超参数网格搜索, 对GCN+LSTM模型和纯LSTM模型进行参数优化。
        - 保存模型的最佳参数到CSV文件。
        - 根据最佳参数创建测试数据集。
        - 评估GCN+LSTM模型和纯LSTM模型, 保存评估结果到列表中。
    b. 将GCN+LSTM模型和纯LSTM模型的评估结果保存到CSV文件中, 每个聚类标签对应一个文件。

'''

# 读取子图数据汇总的数据表，只保留有聚类标签的子图，然后按照聚类标签进行分组
...
for season in tqdm(season, desc='season', unit='season'):
    ...
    for cluster_label, group in tqdm(grouped_subgraphs, desc='cluster', unit='cluster'):
        for group_index in tqdm(range(group.shape[0]), desc='subgraph', unit='subgraph'):
            ...
            adjacency_list = pd.read_csv(fr"D:\Desktop\Data\subgraphs_and_solar_data\{season}\sub_adj_list_{index}.csv").to_numpy()
            roof_solar_array = pd.read_csv(fr"D:\Desktop\Data\subgraphs_and_solar_data\{season}\sub_solar_data_transposed_{index}.csv").to_numpy()
        ...
        df.to_csv(os.path.join(save_path, f'GCN+LSTM_{season}_Cluster_{cluster_label}.csv'))
        # lstm's
        df = pd.DataFrame({'mae': lstm_mae_list, 'mse': lstm_mse_list, 'rmse': lstm_rmse_list, 'mmape': lstm_mmape_list})
        df.to_csv(os.path.join(save_path, f'LSTM_{season}_Cluster_{cluster_label}.csv'))